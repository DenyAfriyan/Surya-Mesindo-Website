    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.blogs.update", [$blog->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('short_description') ? 'is-invalid' : '' }}" type="text" name="short_description" id="short_description" value="{{ old('short_description', $blog->short_description) }}" required>
            ...
            <div class="form-group">
                <label for="author_id">{{ trans('cruds.blog.fields.author') }}</label>
                <select class="form-control select2 {{ $errors->has('author') ? 'is-invalid' : '' }}" name="author_id" id="author_id">
                    @foreach($authors as $id => $entry)
                        <option value="{{ $id }}" {{ (old('author_id') ? old('author_id') : $blog->author->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <label for="likes">{{ trans('cruds.blog.fields.likes') }}</label>
                <input class="form-control {{ $errors->has('likes') ? 'is-invalid' : '' }}" type="number" name="likes" id="likes" value="{{ old('likes', $blog->likes) }}" step="1">
                @if($errors->has('likes'))
                    <span class="text-danger">{{ $errors->first('likes') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.blog.fields.likes_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="shares">{{ trans('cruds.blog.fields.shares') }}</label>
                <input class="form-control {{ $errors->has('shares') ? 'is-invalid' : '' }}" type="number" name="shares" id="shares" value="{{ old('shares', $blog->shares) }}" step="1">
                @if($errors->has('shares'))
                    <span class="text-danger">{{ $errors->first('shares') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.blog.fields.shares_helper') }}</span>
            </div>
            <div class="form-group">
                <label class="required" for="cover">{{ trans('cruds.blog.fields.cover') }}</label>
                <div class="needsclick dropzone {{ $errors->has('cover') ? 'is-invalid' : '' }}" id="cover-dropzone">
                </div>
                @if($errors->has('cover'))
                    <span class="text-danger">{{ $errors->first('cover') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.blog.fields.cover_helper') }}</span>
            </div>
            <div class="form-group">
...
<script>
    ...
    init: function () {
@if(isset($blog) && $blog->cover)
      var file = {!! json_encode($blog->cover) !!}
