    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.spareparts.update", [$sparepart->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" value="{{ old('title', $sparepart->title) }}" required>
                @if($errors->has('title'))
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description">{{ old('description', $sparepart->description) }}</textarea>
...
<script>
    ...
    url: '{{ route('admin.spareparts.storeMedia') }}',
    maxFilesize: 2, // MB
...
@if(isset($sparepart) && $sparepart->image)
      var file = {!! json_encode($sparepart->image) !!}
