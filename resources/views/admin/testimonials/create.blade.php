...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.testimonials.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="testimoni">{{ trans('cruds.testimonial.fields.testimoni') }}</label>
                <textarea class="form-control {{ $errors->has('testimoni') ? 'is-invalid' : '' }}" name="testimoni" id="testimoni">{{ old('testimoni') }}</textarea>
                @if($errors->has('testimoni'))
                    <span class="text-danger">{{ $errors->first('testimoni') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.testimonial.fields.testimoni_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="photos">{{ trans('cruds.testimonial.fields.photos') }}</label>
                <div class="needsclick dropzone {{ $errors->has('photos') ? 'is-invalid' : '' }}" id="photos-dropzone">
                </div>
                @if($errors->has('photos'))
                    <span class="text-danger">{{ $errors->first('photos') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.testimonial.fields.photos_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="company">{{ trans('cruds.testimonial.fields.company') }}</label>
...
<script>
    ...
    init: function () {
@if(isset($testimonial) && $testimonial->photos)
      var file = {!! json_encode($testimonial->photos) !!}
