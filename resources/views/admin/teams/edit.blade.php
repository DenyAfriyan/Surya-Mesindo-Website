...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.teams.update", [$team->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required" for="jobtitle">{{ trans('cruds.team.fields.jobtitle') }}</label>
                <input class="form-control {{ $errors->has('jobtitle') ? 'is-invalid' : '' }}" type="text" name="jobtitle" id="jobtitle" value="{{ old('jobtitle', $team->jobtitle) }}" required>
                @if($errors->has('jobtitle'))
                    <span class="text-danger">{{ $errors->first('jobtitle') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.team.fields.jobtitle_helper') }}</span>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                <label for="photos">{{ trans('cruds.team.fields.photos') }}</label>
                <div class="needsclick dropzone {{ $errors->has('photos') ? 'is-invalid' : '' }}" id="photos-dropzone">
                </div>
                @if($errors->has('photos'))
                    <span class="text-danger">{{ $errors->first('photos') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.team.fields.photos_helper') }}</span>
            </div>
            <div class="form-group">
...
<script>
    ...
    init: function () {
@if(isset($team) && $team->photos)
      var file = {!! json_encode($team->photos) !!}
