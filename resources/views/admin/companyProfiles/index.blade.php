@extends('layouts.admin')
@section('content')
@can('company_profile_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <!-- <a class="btn btn-success" href="{{ route('admin.company-profiles.create') }}">
                {{ trans('global.add') }} {{ trans('cruds.companyProfile.title_singular') }}
            </a>
            <button class="btn btn-warning" data-toggle="modal" data-target="#csvImportModal">
                {{ trans('global.app_csvImport') }}
            </button> -->
            @include('csvImport.modal', ['model' => 'CompanyProfile', 'route' => 'admin.company-profiles.parseCsvImport'])
        </div>
    </div>
@endcan
<div class="card">
    <div class="card-header">
        {{ trans('cruds.companyProfile.title_singular') }} {{ trans('global.list') }}
    </div>

    <div class="card-body">
        <table class=" table table-bordered table-striped table-hover ajaxTable datatable datatable-CompanyProfile">
            <thead>
                <tr>
                    <th width="10">

                    </th>
                    <th>
                        {{ trans('cruds.companyProfile.fields.id') }}
                    </th>
                    <th>
                        {{ trans('cruds.companyProfile.fields.about') }}
                    </th>
                    <th>
                        {{ trans('cruds.companyProfile.fields.name') }}
                    </th>
                    <th>
                        {{ trans('cruds.companyProfile.fields.address') }}
                    </th>
                    <th>
                        {{ trans('cruds.companyProfile.fields.phone') }}
                    </th>
                    <th>
                        {{ trans('cruds.companyProfile.fields.email') }}
                    </th>
                    <th>
                        {{ trans('cruds.companyProfile.fields.fax') }}
                    </th>
                    <th>
                        {{ trans('cruds.companyProfile.fields.instagram') }}
                    </th>
                    <th>
                        {{ trans('cruds.companyProfile.fields.github') }}
                    </th>
                    <th>
                        {{ trans('cruds.companyProfile.fields.linkedin') }}
                    </th>
                    <th>
                        {{ trans('cruds.companyProfile.fields.logo') }}
                    </th>
                    <th>
                        {{ trans('cruds.companyProfile.fields.latitude') }}
                    </th>
                    <th>
                        {{ trans('cruds.companyProfile.fields.longitude') }}
                    </th>
                    <th>
                        &nbsp;
                    </th>
                </tr>
            </thead>
        </table>
    </div>
</div>



@endsection
@section('scripts')
@parent
<script>
    $(function () {
  let dtButtons = $.extend(true, [], $.fn.dataTable.defaults.buttons)
@can('company_profile_delete')
  let deleteButtonTrans = '{{ trans('global.datatables.delete') }}';
  let deleteButton = {
    text: deleteButtonTrans,
    url: "{{ route('admin.company-profiles.massDestroy') }}",
    className: 'btn-danger',
    action: function (e, dt, node, config) {
      var ids = $.map(dt.rows({ selected: true }).data(), function (entry) {
          return entry.id
      });

      if (ids.length === 0) {
        alert('{{ trans('global.datatables.zero_selected') }}')

        return
      }

      if (confirm('{{ trans('global.areYouSure') }}')) {
        $.ajax({
          headers: {'x-csrf-token': _token},
          method: 'POST',
          url: config.url,
          data: { ids: ids, _method: 'DELETE' }})
          .done(function () { location.reload() })
      }
    }
  }
  dtButtons.push(deleteButton)
@endcan

  let dtOverrideGlobals = {
    buttons: dtButtons,
    processing: true,
    serverSide: true,
    retrieve: true,
    aaSorting: [],
    ajax: "{{ route('admin.company-profiles.index') }}",
    columns: [
      { data: 'placeholder', name: 'placeholder' },
{ data: 'id', name: 'id' },
{ data: 'name', name: 'name' },
{ data: 'about', name: 'about' },
{ data: 'address', name: 'address' },
{ data: 'phone', name: 'phone' },
{ data: 'email', name: 'email' },
{ data: 'fax', name: 'fax' },
{ data: 'instagram', name: 'instagram' },
{ data: 'github', name: 'github' },
{ data: 'linkedin', name: 'linkedin' },
{ data: 'logo', name: 'logo', sortable: false, searchable: false },
{ data: 'latitude', name: 'latitude' },
{ data: 'longitude', name: 'longitude' },
{ data: 'actions', name: '{{ trans('global.actions') }}' }
    ],
    orderCellsTop: true,
    order: [[ 1, 'desc' ]],
    pageLength: 100,
  };
  let table = $('.datatable-CompanyProfile').DataTable(dtOverrideGlobals);
  $('a[data-toggle="tab"]').on('shown.bs.tab click', function(e){
      $($.fn.dataTable.tables(true)).DataTable()
          .columns.adjust();
  });
  
});

</script>
@endsection