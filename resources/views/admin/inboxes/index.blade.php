...
@can('inbox_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.inboxes.create') }}">
                {{ trans('global.add') }} {{ trans('cruds.inbox.title_singular') }}
            </a>
            <button class="btn btn-warning" data-toggle="modal" data-target="#csvImportModal">
                {{ trans('global.app_csvImport') }}
            </button>
            @include('csvImport.modal', ['model' => 'Inbox', 'route' => 'admin.inboxes.parseCsvImport'])
        </div>
    </div>
@endcan
<div class="card">
    ...
    <div class="card-body">
        <table class=" table table-bordered table-striped table-hover ajaxTable datatable datatable-Inbox">
            <thead>
                <tr>
                    <th width="10">
...
@can('inbox_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.inboxes.massDestroy') }}",
...
@endcan
  ...
  let dtOverrideGlobals = {
    ...
    retrieve: true,
    aaSorting: [],
    ajax: "{{ route('admin.inboxes.index') }}",
    columns: [
...
{ data: 'actions', name: '{{ trans('global.actions') }}' }
  ...
  let table = $('.datatable-Inbox').DataTable(dtOverrideGlobals);
